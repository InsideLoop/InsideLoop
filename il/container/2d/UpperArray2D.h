...
class UpperArray2D {
 ...
 public:
  UpperArray2D();
  UpperArray2D(il::int_t n);
  UpperArray2D(const UpperArray2D<T>& A);
  UpperArray2D(UpperArray2D<T>&& A);
  UpperArray2D<T>& operator=(const UpperArray2D<T>& A);
  UpperArray2D<T>& operator=(UpperArray2D<T>&& A);
  ~UpperArray2D();
...
template <typename T>
UpperArray2D<T>::UpperArray2D(const UpperArray2D<T>& A) {
...
template <typename T>
UpperArray2D<T>::UpperArray2D(UpperArray2D<T>&& A) {
#ifdef IL_DEBUG_VISUALIZER
...
template <typename T>
UpperArray2D<T>& UpperArray2D<T>::operator=(const UpperArray2D<T>& A) {
  if (this != &A) {
    const il::int_t n{A.size()};
    const il::int_t nb_elements{(n * (n + 1)) / 2};
...
template <typename T>
il::int_t UpperArray2D<T>::size() const {
  return static_cast<il::int_t>(size_ - data_);
}

template <typename T>
il::int_t UpperArray2D<T>::capacity() const {
