...
class Array4D {
  ...
  */
  ...
  //
  // il::Array4D<double> A{4, 6, 7};
  // std::cout << A(3, 5, 6) << std::endl;
  */
  const T& operator()(il::int_t i, il::int_t j, il::int_t k, il::int_t l) const;
  ...
  // A(0, 0, 0) = 3.14;
  */
  T& operator()(il::int_t i, il::int_t j, il::int_t k, il::int_t l);
...
Array4D<T>::Array4D(il::int_t n0, il::int_t n1, il::int_t n2, il::int_t n3) {
  ...
  IL_ASSERT(n2 >= 0);
  IL_ASSERT(n3 >= 0);
  il::int_t np{n0 > 0 && n1 > 0 && n2 > 0 && n3 > 0
                   ? n0 * n1 * n2 * n3
                   : (n0 > n1 && n0 > n2 && n0 > n3
                          ? n0
  ...
  if (std::is_pod<T>::value) {
    ...
    data_ = static_cast<T*>(::operator new(np * sizeof(T)));
    for (il::int_t l{0}; l < n0 * n1 * n2 * n3; ++l) {
      new (data_ + l) T{};
    }
  }
  ...
  debug_capacity_2_ = n2;
  debug_capacity_3_ = n3;
#endif
  size_[0] = data_ + n0;
  size_[1] = data_ + n1;
  size_[2] = data_ + n2;
  size_[3] = data_ + n3;
  capacity_[0] = data_ + n0;
  capacity_[1] = data_ + n1;
  capacity_[2] = data_ + n2;
  capacity_[3] = data_ + n3;
}
...
template <typename T>
const T& Array4D<T>::operator()(il::int_t i, il::int_t j, il::int_t k) const {
...
template <typename T>
T& Array4D<T>::operator()(il::int_t i, il::int_t j, il::int_t k) {
  IL_ASSERT(static_cast<il::uint_t>(i) < static_cast<il::uint_t>(size(0)));
  IL_ASSERT(static_cast<il::uint_t>(j) < static_cast<il::uint_t>(size(1)));
  IL_ASSERT(static_cast<il::uint_t>(k) < static_cast<il::uint_t>(size(2)));
  return data_[(i * stride(0) + j) * stride(1) + k];
...
template <typename T>
void Array4D<T>::resize(il::int_t n, il::int_t p, il::int_t q) {
  IL_ASSERT(n >= 0);
  IL_ASSERT(p >= 0);
  IL_ASSERT(q >= 0);
...
template <typename T>
void Array4D<T>::reserve(il::int_t r, il::int_t s, il::int_t t) {
  IL_ASSERT(r >= 0);
  IL_ASSERT(s >= 0);
  IL_ASSERT(t >= 0);
...
template <typename T>
il::int_t Array4D<T>::stride(il::int_t d) const {
