...
class Gmres {
 ...
 public:
  ...
  void Solve(il::ArrayView<T> y, double relative_precision,
             il::int_t max_nb_iterations, il::io_t, il::ArrayEdit<T> x, il::Status& status);
...
void Gmres<T>::Solve(il::ArrayView<T> y, double relative_precision,
                     il::int_t max_nb_iterations, il::io_t,
                     il::ArrayEdit<T> x, il::Status& status) {
  ...
  dfgmres_init(&n_, x.data(), yloc_.data(), &RCI_request, ipar_.Data(),
  ...
  dfgmres_check(&n_, x.data(), yloc_.data(), &RCI_request, ipar_.Data(),
                dpar_.Data(), tmp_.Data());
  IL_EXPECT_FAST(RCI_request == 0);
  ...
  while (!stop_iteration) {
    ...
    dfgmres(&n_, x.Data(), yloc_.Data(), &RCI_request, ipar_.Data(),
            dpar_.Data(), tmp_.Data());
    switch (RCI_request) {
      case 0:
      ...
      case 2: {
        ...
        dfgmres_get(&n_, x.Data(), ycopy_.Data(), &RCI_request, ipar_.data(),
  ...
  ipar_[12] = 0;
  dfgmres_get(&n_, x.Data(), yloc_.Data(), &RCI_request, ipar_.data(),
              dpar_.data(), tmp_.Data(), &itercount);
