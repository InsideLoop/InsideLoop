...
il::Dynamic TomlParser::parseBoolean(il::io_t, il::ConstStringView& string,
                                     Status& status) {
...
il::String TomlParser::parseStringLiteral(char delimiter, il::io_t,
  ...
  while (!string.isEmpty()) {
    if (delimiter == '"' && string.hasAscii(0, '\\')) {
      ...
      ans.append(string.toUnicodeScalar(0));
      string.removePrefix(string.nextUnicodeScalar(0));
    }
  }
...
il::Dynamic TomlParser::parseValue(il::io_t, il::ConstStringView& string,
  ...
  switch (type) {
    case il::Type::kBool:
      ans = parseBoolean(il::io, string, parse_status);
      break;
    case il::Type::kInteger:
    case il::Type::kDouble:
