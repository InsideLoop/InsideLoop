  ...
  while (true) {
    ...
    switch (type) {
      ...
      case il::Type::kString: {
        ...
        k += size + 1;
        il::String value{il::StringType::kByte, raw_value.data(),
                         raw_value.size()};
      ...
      case il::Type::kArrayOfStruct: {
        ...
        for (il::int_t i = 0; i < nb_types; ++i) {
          switch (type[i]) {
            case il::Type::kDouble: {
              array[i] = il::Dynamic{il::Type::kArrayOfDouble};
              il::Array<double>& ref = array[i].as<il::Array<double>>();
              ref.Resize(n);
            } break;
            case il::Type::kInteger: {
              array[i] = il::Dynamic{il::Type::kArrayOfInt32};
...
inline void auxLoad(il::int_t n, il::io_t,
  ...
  while (true) {
    ...
    il::String string{il::StringType::kByte, raw_string.data(), size_string};
    k += string.size();
    ...
    switch (type) {
      ...
      case il::Type::kString: {
        ...
        k += size;
        il::String value{il::StringType::kByte, raw_value.data(), size};
        config.Set(std::move(string), il::Dynamic{std::move(value)});
      } break;
      case il::Type::kArrayOfDouble: {
        il::int_t size = 0;
      ...
      case il::Type::kMapArray: {
        if (n == -1) {
          ...
          for (il::int_t i = 0; i < nb_types; ++i) {
            ...
            il::Array<char> raw_string{key_size};
            std::fread(raw_string.data(), sizeof(char), key_size, file);
            il::String my_string{il::StringType::kByte, raw_string.data(),
                                 key_size};
            key[i] = my_string;
...
inline void auxSave(const il::MapArray<il::String, il::Dynamic>& data,
  ...
  for (il::int_t i = 0; i < data.size(); ++i) {
    il::String to_check{il::StringType::kByte, data.key(i).asCString(),
                        data.key(i).size()};
          ...
          for (il::int_t i = my_data.begin(); i < my_data.end();
            ...
            switch (type) {
              case il::Type::kArrayOfDouble: {
                ...
                const il::Type my_type = il::Type::kDouble;
              ...
              case il::Type::kArrayOfInt32: {
                ...
                const il::Type my_type = il::Type::kInteger;
                std::fwrite(&my_type, sizeof(il::Type), 1, file);
              } break;
              default:
                IL_UNREACHABLE;
