...
PartialLU::PartialLU(il::Array2D<double> A, il::io_t, il::Error& error)
    : ipiv_{}, lu_{} {
...
il::int_t PartialLU::size(il::int_t d) const {
  IL_ASSERT_BOUNDS(static_cast<il::uint_t>(d) < static_cast<il::uint_t>(2));
  return lu_.size(d);
}

il::Array<double> PartialLU::solve(il::Array<double> y) const {
  IL_ASSERT(lu_.size(0) == lu_.size(1));
...
il::Array2D<double> PartialLU::solve(il::Array2D<double> y) const {
  IL_ASSERT(lu_.size(0) == lu_.size(1));
  IL_ASSERT(lu_.size(0) == y.size(0));
...
il::Array2D<double> PartialLU::inverse() const {
  IL_ASSERT(lu_.size(0) == lu_.size(1));
...
double PartialLU::condition_number(il::Norm norm_type, double norm_a) const {
  IL_ASSERT(lu_.size(0) == lu_.size(1));
  IL_ASSERT(norm_type == il::Norm::L1 || norm_type == il::Norm::Linf);
...
const double& PartialLU::L(il::int_t i, il::int_t j) const {
  IL_ASSERT_BOUNDS(j < i);
  return lu_(i, j);
}

const double& PartialLU::U(il::int_t i, il::int_t j) const {
  IL_ASSERT_BOUNDS(j >= i);
  return lu_(i, j);
}

il::LowerArray2D<double> PartialLU::L() const {
  IL_ASSERT(size(0) == size(1));
...
il::UpperArray2D<double> PartialLU::U() const {
  IL_ASSERT(size(0) == size(1));
